...
from chatbot.chat import classify_input
from pydantic import BaseModel
from urllib.parse import unquote
from fastapi.encoders import jsonable_encoder
from fastapi.responses import JSONResponse

app = FastAPI(default_encoding="utf-8")
...
async def chatbot(req: str):
    ...
    :return:
    """
    decoded_text = unquote(req, encoding='utf-8').replace("+", " ")
    res = classify_input(decoded_text)
    with db_session:
        for c in Category.select():
            if c.name == res:
                return {"answers": [ck.name for ck in c.sub_categories]}
        for c in AtmCategory.select():
            if c.name == res:
                return {"answers": [ck.name for ck in c.atm_sub_categories]}
        return {"answers": [ck.name for ck in c.atm_sub_categories]}
